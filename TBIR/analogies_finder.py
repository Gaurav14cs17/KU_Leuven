...
def find_analogies(input_file, ouptut_file):
    ...
    possible_outputs_dict = build_outputs_list(ouptut_file) #d'
    ...
    with open(input_file) as fd_inputs:  #
        for input in fd_inputs:
            ...
            if category_bool is False:
                print 'output(mult) found: ', output_mult, 'took:', time.time()-start
                print 'output(add) found: ', output_add, 'took:', time.time()-start
                print 'output(dir) found: ', output_dir, 'took:', time.time()-start
