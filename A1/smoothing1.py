...
def gaussian_filter(sigma, filter_length=None):
    ...
    G(x;sigma) = 1/sqrt(2pi*sigma)*e**(-x**2)/2sigma**2
    """
    mean = int(filter_length/2)
    for i in range(filter_length):
        # gaussian filter not "centered" so we have to take away the mean the center
        x = i-mean
        result[i] = (1/math.sqrt(2*math.pi*sigma))*math.exp(-(x)**2/(2.0*sigma**2))
        #print result[i]
    #return the filter
    # normalize the result
    normalize = np.sum(result)
...
def gaussian_smooth1(img, sigma):
    ...
    # img.shape(rows, columns, colors)
    for c in range(3):
        #smooth the 2D image img[:,:,c]
        #tip: make use of numpy.convolve
        # rows
        #np.convolve array1, array2, mode {same, valid, full}
        for i in range(img.shape[0]):
            result[i,:,c] = np.convolve(img[i,:,c], filter, mode='same')
        # columns
        for j in range(img.shape[1]):
            result[:,j,c] = np.convolve(result[:,j,c], filter, mode='same')
