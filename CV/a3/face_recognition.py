...
def create_database(directory, show = True):
    ...
    for filename in fnmatch.filter(os.listdir(directory),'*.jpg'):
        ...
        result = cv2.resize(img[rects[1]:rects[3], rects[0]:rects[2]], (100,100), interpolation=cv2.INTER_AREA)#TOdo
...
def pca(X, nb_components=0):
    ...
    print 'mean_vect shape:',mean_vec.shape

    cov_mat = np.cov(np.transpose(X-mean_vec))
    print 'Covariance matrix shape: ', cov_mat.shape

    eig_vals, eig_vecs = sLA.eigsh(cov_mat, k=nb_components, which='LA')
    #print eig_vals
    ...
    for i in range(len(eig_pairs)):
        eigvals.append(eig_pairs[i][0])
        eigvecs.append(eig_pairs[i][1])

    print('Eigenvalues in descending order:')
    print np.asarray(eigvals)

    return np.asarray(eigvals), np.asarray(eig_vecs), mean_vec
